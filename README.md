...
### Контакты
По всем вопросам можно связаться по почте: [pwsdotru@gmail.com](mailto:pwsdotru@gmail.com) 
или Telegram: [@pwsdotru](https://t.me/pwsdotru).

Новости и апдейты в канале Telegram [@spawning_geo](https://t.me/spawning_geo) 
...
Для разбора файла документов создан скрипт на Python [parser.py](./console/parser.py) 
При запуске в качестве аргумента командной строки передает путь до файла, который надо обработать.
Результат выводится в консоль в виде geojson.

Пример запуска

````
python console/parser.py examples/full-list.odt
```` 

Возможны параметры:

--format raw|geojson - формат в котором выводить результат. 

**raw** в формате JSON выводятся все данные, которые были получены в результате парсинга

**geojson** в формате geoJSON, совместимым с Яндекс Картами и т.п.
...
Преобразование координат делается по формуле

````
результат = гг + мм/60 + ссс/3600
````
...
Найденны ошибки собраны в [отдельном файле](./docs/errors.md)
